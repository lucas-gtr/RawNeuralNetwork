...
Various usage examples of this repository are accessible in the [examples folder](src/examples)

### XOR example

To illustrate the usage, let's apply it to the XOR problem (a logical operation for exclusive OR).

Firstly, import the entire neural network components using the raw_nn import.
...
Ensure the input shape is (sample size, input features), and the output shape can be (sample size) or (sample size, output dimension). In this case, we have 4 elements in our dataset, 2 input features, and one output feature (1 corresponds to True and 0 to False). Therefore, the shape of X is (4, 2), and the shape of y is (4).
...
The model provides an add method to append new layers. Let's create a model with a dense layer using ReLU activation, followed by another dense layer with sigmoid activation:
...
Various optimizers are available; to create one, simply pass the model parameters as arguments and define a custom learning rate. Here, we'll use the `SGD` optimizer:
...
The forward propagation occurs in the order in which the layers were added. 
Subsequently, compute the loss using loss_fn, passing the predicted output and the true output:
...
We observe a decreasing loss. If we print the output of the model with the input data, it closely matches our expectations.
...
The complete file for this example is available in the [xor_example file](src/examples/xor_example.py).
...
By plotting them, we obtain this graphic : 

<img width="581" alt="Spiral plot" src="https://github.com/lucas-gtr/RawNeuralNetwork/assets/12534925/3614c97f-2cff-4fed-957b-d236f7ad4b82">
